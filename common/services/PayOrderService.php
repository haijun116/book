...
class PayOrderService extends BaseService
{
    ...
    /**
     * @param $member_id
     * @param array $items 商品信息book_id,quantity
     * @param array $params 额外提供的商品信息 pay_type,pay_source,target_type
     */
    ...
    public static function orderSuccess($pay_order_id, $params = [])
    {
        ...
        try {
            ...
            if (!$pay_order_info || in_array($pay_order_info['status'], [-7, -8])) {
                return true;
            }
            $pay_order_info->pay_sn = isset($params['sn']) ? $params['sn'] : '';
